...
here — team allocation, founder revenue, bootstrap multisig, audit status — but
framed as things worth checking, not things being confessed. An earlier draft led
five of eleven tweets with a limitation; it read like an apology and buried
genuinely strong facts (under-1% team allocation, full CI coverage, no possible
death spiral).

Supersedes the earlier build-in-public runway (teaser → Posts 1–6) — all prior
tweets deleted, this is the full replacement. Post once the operational gates in
`docs/MAINNET_LAUNCH.md` are green. ⚠️ Tweets 1 and 11 say "live" and "Round 1 is
open" — only send after Steps 1–2 are actually done and the mint exists.
...
>
> A hard $1 ceiling, enforced on-chain. 1,024 mint rounds at one fixed price. No presale, no VC round, no private allocation.
>
> Fixed supply. Burned liquidity. All of it verifiable.
>
> tobestable.com
...
>
> Below $1 the vault buys back using a real SOL reserve that grows every round: bounded by honest reserves, not by printing.
...
>
> Self-audited across 8 vulnerability classes and published in full — findings, fixes and scope. It's an AI-assisted audit, not a paid firm, and the report says so plainly.
>
